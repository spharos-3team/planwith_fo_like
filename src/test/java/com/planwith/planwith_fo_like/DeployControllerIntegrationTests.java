...
class DeployControllerIntegrationTests {
	...
	void deployCheckReturnsMarker() throws Exception {
		mockMvc.perform(get("/api/planwith-fo-like/deploy-check"))
				.andExpect(status().isOk())
				.andExpect(jsonPath("$.service").value("planwith-fo-like"))
				.andExpect(jsonPath("$.marker").value("planwith-fo-like-deploy-v1"))
				.andExpect(jsonPath("$.message").value("planwith-fo-like deploy pipeline ok"));
	}

	@Test
	void loginSucceedsWithConfiguredCredentials() throws Exception {
		mockMvc.perform(post("/api/planwith-fo-like/login")
