...
@ConfigurationProperties(prefix = "like.outbox")
public class LikeOutboxProperties {
	...
	public Instant nextRetryAt(Instant now, int retryCount) {
		...
		return now.plus(Duration.ofMillis(Math.min(max.toMillis(), Math.max(initial.toMillis(), millis))));
	}
